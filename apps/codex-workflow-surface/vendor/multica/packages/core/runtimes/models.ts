/* CCP modification: Keep native model selection authoritative without daemon discovery. Upstream attribution: vendor/multica/NOTICE. */
...
export async function resolveRuntimeModels(
  ...
  const runtime = (await api.listRuntimes()).find((entry) => entry.id === runtimeId);
