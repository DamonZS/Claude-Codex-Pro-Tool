/* CCP modification: Accept authoritative local catalog capability without changing membership; expose query errors and retry. Upstream attribution: vendor/multica/NOTICE. */
"use client";
...
export function PropertiesTab({ localCanManage, managementHint }: { localCanManage?: boolean; managementHint?: string } = {}) {
  ...
  const canManage = localCanManage ?? (currentMember?.role === "owner" || currentMember?.role === "admin");
  ...
  return (
    ...
    >
      <div className="space-y-5">
        ...
        {!canManage && (
          <p className="text-caption text-muted-foreground">
            {managementHint ?? t(($) => $.properties.editor.admin_hint)}
          </p>
        )}
      ...
      <PropertyEditorDialog managementHint={managementHint} open={createOpen} onOpenChange={setCreateOpen} />
