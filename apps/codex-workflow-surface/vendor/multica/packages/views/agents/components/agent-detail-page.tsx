/* CCP modification: Expose a host return control for agent detail and loading state. Upstream attribution: vendor/multica/NOTICE. */
"use client";
...
export function AgentDetailPage({ agentId, leadingAction }: AgentDetailPageProps) {
  const { t } = useT("agents");
  ...
  if (!agent && (agentsLoading || detailQuery.isPending)) {
    return <DetailLoadingSkeleton leading={leadingAction} />;
  }
...
function DetailLoadingSkeleton({ leading }: { leading?: ReactNode } = {}) {
  return (
    <div className="flex flex-1 min-h-0 flex-col">
