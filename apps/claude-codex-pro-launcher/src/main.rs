...
fn acquire_single_instance_guard(
    debug_port: u16,
) -> anyhow::Result<Option<claude_codex_pro_core::ports::LoopbackPortGuard>> {
...
fn log_launcher_guard_fallback(fallback_lock_path: &Path) {
    let _ = claude_codex_pro_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "requested_guard_port": claude_codex_pro_core::ports::LAUNCHER_GUARD_PORT,
...
async fn activate_existing_codex_app(options: &LaunchOptions) -> anyhow::Result<()> {
    ...
    let result = async {
        let process_ids = claude_codex_pro_core::watcher::find_codex_processes();
        let activated = {
            #[cfg(windows)]
            {
                let mut activated = false;
                for process_id in &process_ids {
                    if claude_codex_pro_core::windows_activate_process_window(*process_id) {
                        activated = true;
                        break;
                    }
                }
...
fn log_launcher_already_running(debug_port: u16) {
    let _ = claude_codex_pro_core::diagnostic_log::append_diagnostic_log(
        ...
        json!({
            "guard_port": claude_codex_pro_core::ports::LAUNCHER_GUARD_PORT,
            "debug_port": debug_port
        }),
    );
}
...
impl LaunchHooks for LauncherHooks {
    ...
    async fn launch_codex(
        ...
        debug_port: u16,
        extra_args: &[String],
    ) -> anyhow::Result<claude_codex_pro_core::launcher::CodexLaunch> {
        self.core
    ...
    async fn wait_for_codex_exit(
        &self,
        launch: &claude_codex_pro_core::launcher::CodexLaunch,
    ) -> anyhow::Result<()> {
    ...
    async fn terminate_codex(&self, launch: &claude_codex_pro_core::launcher::CodexLaunch) {
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: claude_codex_pro_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl BridgeDataService for LauncherDataService {
    async fn delete(&self, session: SessionRef) -> anyhow::Result<DeleteResult> {
        ...
        tokio::task::spawn_blocking(move || {
            claude_codex_pro_data::delete_local_from_paths(db_paths, backup_store, &session)
        })
        .await
    ...
    async fn export_markdown(&self, session: SessionRef) -> anyhow::Result<ExportResult> {
        let export_service =
            claude_codex_pro_data::MarkdownExportService::new(Some(self.db_path.clone()));
...
impl LauncherDataService {
    ...
    fn storage_adapter(&self) -> claude_codex_pro_data::SQLiteStorageAdapter {
        claude_codex_pro_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            claude_codex_pro_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn reload_user_scripts(&self) -> anyhow::Result<Value> {
        ...
        if let Some(websocket_url) = websocket_url.filter(|_| !bundle.trim().is_empty()) {
            claude_codex_pro_core::bridge::evaluate_script(&websocket_url, &bundle).await?;
        }
        self.user_scripts.inventory()
    }
    ...
    async fn codex_model_catalog(&self) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::model_catalog::read_codex_model_catalog().await)
    }

    async fn ads(&self) -> anyhow::Result<Value> {
        claude_codex_pro_core::ads::fetch_ad_list().await
    }

    async fn zed_remote_status(&self) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::zed_remote::zed_remote_status())
    }
    ...
    async fn open_zed_remote(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::zed_remote::open_zed_remote(&payload))
    }

    async fn list_zed_remote_projects(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::zed_remote::list_zed_remote_projects_response(&payload))
    }

    async fn remember_zed_remote_project(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::zed_remote::remember_zed_remote_project_response(&payload))
    }

    async fn forget_zed_remote_project(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::zed_remote::forget_zed_remote_project_response(&payload))
    }

    async fn upstream_worktree_status(&self) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::upstream_worktree::status_response())
    }

    async fn upstream_worktree_defaults(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::upstream_worktree::defaults_response(
            &payload,
        ))
    }

    async fn upstream_worktree_prepare(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(claude_codex_pro_core::upstream_worktree::prepare_response(
            &payload,
        ))
    }
...
async fn try_inject_with_context(
    ...
    claude_codex_pro_core::bridge::install_bridge(
        ...
        Arc::new(move |path, payload| {
            let ctx = ctx.clone();
            Box::pin(async move {
                Ok(claude_codex_pro_core::routes::handle_bridge_request(ctx, &path, payload).await)
            })
        }),
...
fn default_codex_db_path() -> PathBuf {
    claude_codex_pro_core::codex_sqlite::codex_session_db_path()
}
