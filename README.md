...
- 支持通过前端注入解锁模型白名单，修复第三方 API 已支持 GPT-5.6、但 Codex 模型菜单仍不可选的问题。
- 支持服务等级控制入口。
- 支持图片覆盖配置。
- 支持会话滚动位置恢复。
- 支持会话时间线和会话视图增强。
...
### 第三方 API 已支持 GPT-5.6，为什么 Codex 里仍不能选择？

部分 Codex 版本会继续使用官方前端模型白名单，即使当前第三方 API 已经返回 GPT-5.6，模型菜单也可能只显示旧模型。CCP 的“模型白名单解锁”会通过注入修补 Codex 的模型目录响应、模型请求链路和模型选择菜单，并加入当前供应商中已保存的 `gpt-5.6` / `gpt-5.6-*`。

使用步骤：

1. 在管理工具的供应商页面获取模型，或把上游真实支持的 GPT-5.6 模型 ID 加入当前供应商模型列表，然后点击“保存并使用”。
2. 在设置中确认“模型白名单解锁”已开启；该功能默认开启。
3. 从 `Claude Codex Pro` 入口启动或重启 Codex。若 Codex 已经运行，可在管理工具中执行“修复前端连接”，等待注入标识恢复后再打开模型菜单。
4. 重新打开模型选择器，选择对应的 GPT-5.6 模型。

模型目录还兼容 Codex `config.toml` 中的 `model_catalog_json`，以及 `~/.codex/model-catalog.gpt-5.6.json` / `~/.codex/model-catalog.json`。注入只解除客户端“看不见、选不了”的限制，不会让原本不支持 GPT-5.6 的 API 获得该模型；如果选择后请求失败，应检查供应商模型 ID、协议和上游可用性。
