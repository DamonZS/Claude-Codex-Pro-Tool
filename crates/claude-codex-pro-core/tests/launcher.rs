...
use claude_codex_pro_core::ports::{
    select_packaged_codex_debug_port_with, select_platform_loopback_port_with,
};
use claude_codex_pro_core::settings::{BackendSettings, RelayProfile, RelayProtocol};
...
#[test]
fn app_paths_find_latest_windows_package_ignores_non_codex_packages() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(temp.path().join("Contoso.Other_99.0.0.0_x64__abc/app")).unwrap();
    std::fs::create_dir_all(temp.path().join("OpenAI.Codex_26.611.8604.0_x64__abc/app")).unwrap();

    let latest = find_latest_codex_app_dir(temp.path()).unwrap();

    assert_eq!(
        latest,
        temp.path().join("OpenAI.Codex_26.611.8604.0_x64__abc/app")
    );
    assert_eq!(codex_app_version(&latest).as_deref(), Some("26.611.8604.0"));
}
...
#[test]
fn launch_options_default_debug_port_does_not_conflict_with_claude_inspector() {
    assert_eq!(LaunchOptions::default().debug_port, 9230);
}
...
#[test]
fn launcher_keeps_packaged_activation_alive_when_cdp_is_not_ready() {
    let source =
        std::fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("src/launcher.rs"))
            .unwrap();

    assert!(source.contains("launcher.packaged_activation_cdp_unready_degraded"));
    assert!(!source.contains("launcher.packaged_activation_cdp_unready_direct_fallback"));
    let degraded_index = source
        .find("launcher.packaged_activation_cdp_unready_degraded")
        .unwrap();
    let after_degraded = &source[degraded_index..];
    let return_index = after_degraded.find("return Ok(packaged_launch);").unwrap();
    let terminate_index = after_degraded.find("terminate_windows_process_id(process_id)");
    assert!(match terminate_index {
        Some(index) => index > return_index,
        None => true,
    });
}
...
async fn launch_lifecycle_runs_sync_before_launch_writes_success_and_shutdowns_on_exit() {
    ...
    let hooks = FakeHooks::new(events.clone())
        ...
        .with_launch_result(CodexLaunch::Process {
            ...
            wait_strategy: claude_codex_pro_core::launcher::ProcessWaitStrategy::TrackedChild,
...
async fn launch_lifecycle_keeps_js_injection_in_relay_mode() {
    ...
    let events = Arc::new(Mutex::new(Vec::<String>::new()));
    let hooks = FakeHooks::new(events.clone()).with_settings(BackendSettings {
        launch_mode: claude_codex_pro_core::settings::LaunchMode::Relay,
        ..BackendSettings::default()
    });
...
async fn protocol_proxy_port_uses_selected_helper_port_when_default_is_busy() {
    ...
    let settings = BackendSettings {
        ...
        relay_profiles: vec![RelayProfile {
            ...
            relay_mode: claude_codex_pro_core::settings::RelayMode::MixedApi,
            official_mix_api_key: false,
            test_model: String::new(),
            config_contents: String::new(),
            auth_contents: String::new(),
            use_common_config: true,
            context_selection: claude_codex_pro_core::settings::RelayContextSelection::default(),
            context_selection_initialized: false,
            context_window: String::new(),
            auto_compact_limit: String::new(),
            model_insert_mode: claude_codex_pro_core::settings::RelayModelInsertMode::default(),
