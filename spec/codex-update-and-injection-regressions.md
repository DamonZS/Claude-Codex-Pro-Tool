...
2. Codex 注入弹窗曾被模型名称或模型下拉相关浮层污染；模型选择注入现已废弃，本任务只要求弹窗不创建或接收 CCP 模型增强项。
...
- 确认 Codex 模型选择注入已移除，避免任何模型菜单逻辑再次污染 CCP 控制弹窗。
...
- 用户打开 Codex 注入弹窗时，底部区域不再被 CCP 模型列表覆盖，弹窗内部不出现模型增强项；Codex 原生模型菜单仍由 Codex 自身渲染。
...
- 不得扫描 Codex 模型菜单、安装模型 JSON / Statsig / React / App Server 补丁，或追加任何 CCP 模型项。
- CCP 自身弹窗、普通菜单和页面内容均不得接收模型候选或请求模型覆盖。
- Codex 原生模型菜单仍由 Codex 自身渲染；模型列表与选择行为不属于本任务的增强范围。具体负向契约以 [`spec/remove-codex-model-selection-injection.md`](remove-codex-model-selection-injection.md) 为准。
