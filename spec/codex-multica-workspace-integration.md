...
- 在 Codex App 左侧原生“插件”入口下方插入三个单实例工作区入口：`我的任务`、`自动化`、`智能体`，分别进入既有的 `my-issues`、`autopilots`、`agents` 路由。Codex 原生“项目”区域保持原位且不被 CCP 复制、替换或插入第二个入口；Skills 保留在工作区紧凑模块菜单中，不在侧栏重复展示。
- 在 Codex 主内容区直接展示 CCP 自有、本地打包、非 iframe 的全宽任务看板；不再叠加 `Multica 工作区 / Local Multica Workspace` 顶部壳、永久模块侧栏或“关闭”按钮。
- 首批完整提供九个 CCP 工作区模块：`我的任务`、`任务`、`自动化`、`智能体`、`小队`、`统计`、`运行时`、`Skills`、`设置`。`项目`不属于 CCP 工作区模块，继续使用 Codex 原生项目区域。
- 三个工作流高频模块通过左侧插件区的独立入口直接进入；Codex 原生项目继续由原生“项目”区域打开。`任务`、`小队`、`统计`、`运行时`、`Skills`、`设置`仍通过看板工具栏中的紧凑模块菜单和上下文动作进入。所有模块保留同一稳定路由和完整能力，不以第二套永久内容导航挤占主内容宽度。
...
1. CCP 启动并注入 Codex 后，在原生“插件”按钮下方依次显示 `我的任务`、`自动化`、`智能体`；Codex 原生“项目”区域保持原有位置，CCP 不插入“项目”或“Skill”侧栏项。
...
3. 点击左侧三个入口之一后，直接打开其对应页面；`我的任务`默认进入 `my-issues` 七列看板。Codex 项目只由原生项目行打开；其余模块通过看板工具栏中的紧凑模块菜单或上下文动作进入，不显示永久模块侧栏。
...
6. 本地 bridge 未安装、Launcher 未启动、binding 缺失、请求超时或本地传输失败时，点击“我的任务”不得冻结入口。入口可立即接管主内容并显示真实加载态；数据请求失败时保留看板并显示可恢复的不可用状态和直接重试，不得把失败渲染成“无任务”。

### 2. 创建和分配任务

1. 用户在“任务”中创建 Issue，至少输入标题；描述、优先级、父任务、日期、位置、自定义状态均为可选项。既有 `project_id` 只保留为内部历史关联字段，不提供工作流项目详情或 CRUD。
   在“我的任务”七列看板中点击任一列头的“新建”动作，必须打开同一套任务编辑器，并将该列的状态预填入草稿。
...
工作区内部使用稳定路由键，不接管 Codex 自身 URL 路由。Codex 原生“插件”下方提供三个高频模块的独立入口，顺序固定为 `我的任务`、`自动化`、`智能体`。`项目`始终是 Codex 原生项目区域，不创建 CCP 侧栏入口；`Skills`仅通过紧凑模块菜单进入。`任务`、`小队`、`统计`、`运行时`、`设置`同样通过看板工具栏中的单个紧凑模块菜单或业务上下文动作进入。禁止渲染第二个永久内容竖向模块栏。工作流模块在紧凑菜单中的顺序固定如下：

| 顺序 | 模块 | 路由键 | 最低可用能力 |
| --- | --- | --- | --- |
| 1 | 我的任务 | `my-issues` | 当前身份相关任务、筛选、打开、继续、个人负载与待处理状态 |
| 2 | 任务 | `issues` | 列表/看板、创建、编辑、分配、拖拽、父子任务、执行和 attempt 历史 |
| 3 | 自动化 | `autopilots` | schedule/webhook/api 触发定义、启停、手动运行、run 历史和失败诊断 |
| 4 | 智能体 | `agents` | 智能体定义、启停、能力/Skills、并发策略和 Codex 原生执行映射状态 |
| 5 | 小队 | `squads` | 负责人、成员、分工、父子执行拓扑、队列和汇总状态 |
| 6 | 统计 | `usage` | Issue、attempt、成功率、耗时、队列与智能体/小队维度的审计统计 |
| 7 | 运行时 | `runtimes` | 本地 Multica 控制面与当前 Codex 页面原生能力的只读状态、版本和故障信息；不管理或注册进程 |
| 8 | Skills | `skills` | 可用 Codex Skill、来源、信任/安装状态、任务/智能体绑定、页面原生能力和执行加载结果；不隐式安装 |
| 9 | 设置 | `settings` | 持久化工作区启用/停用开关、workspace 默认值、分配/派发策略、通知、并发与同步设置；高级运维打开 Manager |
...
- 使用插件按钮父节点的连续 `insertBefore`，确保 DOM 顺序和视觉顺序均为“插件 → 我的任务 → 自动化 → 智能体 → Codex 原生项目区域”。
- 每个入口使用稳定的 `data-ccp-multica-nav-route`、共享入口标识和注入版本号；重复扫描、窗口切换、React 重绘和重新注入后每个路由全页最多存在一个有效入口。
...
- 所有入口具备可见 focus、键盘激活、选中状态和可读无障碍名称，尺寸与 Codex 原生导航一致；侧栏空间不足时由原生侧栏垂直滚动，不裁剪或隐藏后续入口。
...
- 点击“我的任务”后必须立即接管内容区并显示真实的“正在连接/正在读取任务”状态；不得串行等待 bootstrap 与首个查询完成后才显示界面。
- bootstrap 与 `my-issues` 查询在后台完成，成功后更新看板；失败时在看板内显示可读错误和“重试”，不得冻结入口或伪装成“无任务”。
- `my-issues` 正文只包含七列任务看板及其工具栏、任务卡和必要状态提示。原生项目、会话、事件、Skill、智能体、收件箱、自动化和队列等库存不得全量插入看板正文，应从对应独立模块访问。
...
- 在插件下方、原生项目上方插入三个高频入口，完成重注入、卸载和响应式页面壳。
- 接入 bootstrap、workspace 选择、十模块导航及各模块真实加载/空/错/过期态。
- 运行最窄的注入契约、前端类型检查和 bridge 只读测试。

### 阶段 2：Issue、项目和看板写入

- 实现 Issue CRUD、分配、父子关系、revision/CAS 和拖拽回滚；任务的 `project_id` 仅保留为既有内部关联字段，不构成左侧“项目”入口的数据来源或 CRUD 能力。
- 增加并发冲突、重复提交、离线和权限测试。
