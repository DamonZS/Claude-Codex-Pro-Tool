...
3a. 显式提炼长期记忆入口可用
   - 通过标准：管理工具存在“提炼长期记忆”按钮。
   - 通过标准：按钮触发带修复/学习能力的后端链路，而不是仅刷新前端状态。
   - 通过标准：提炼后刷新长期记忆列表、待确认数量、采集证据和注入摘要缓存。
   - 证据：manager 定向测试或源码断言通过；必要时结合 Rust 记忆测试验证高置信历史消息可进入长期记忆。
...
5. 学习原因与未触发原因可见
   ...
   - 通过标准：复制出来的 PowerShell/Git 临时错误输出不会自动写入长期记忆，只会作为不可学习采集证据保留。
   - 通过标准：高置信经验教训、踩坑复盘和用户纠错后的稳定规则可自动进入长期记忆，分类为 `lesson-learned`。
   - 通过标准：经验教训类长期记忆文本带“经验教训：”前缀，且保留可执行动作和上下文对象。
   - 通过标准：无上下文抱怨、一次性状态、终端错误输出不能被误写入长期记忆。
