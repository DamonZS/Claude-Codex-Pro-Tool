...
9. `codex_native_events` 只读集合最多 1000 条，每项含线程、事件 ID、类型、序号、时间和可选 160 字符内摘要；不得包含完整提示词、工具参数、命令输出或凭据。
10. 原生线程关系扫描先覆盖有界历史窗口（5000 条），再截断展示；当子线程早于最近 100 条时，仍能依据 `thread_spawn_edges` 投影为原生子智能体。
...
- 使用本机 `state_5.sqlite` 统计 `threads` 与 `thread_spawn_edges`，确认扫描窗口覆盖关系表中的子线程；运行 Release 后在“我的任务”核对原生智能体数量。
